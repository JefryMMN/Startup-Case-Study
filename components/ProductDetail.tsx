            ...
            <section>
              ...
              <ul className="space-y-6">
                {study.keyMistakes.map((mistake, idx) => (
                  <li key={idx} className="flex gap-5 items-start bg-white p-6 rounded-lg shadow-sm border border-[#EBE7DE]">
                    <span className="flex-shrink-0 w-8 h-8 rounded-full bg-[#FFEBEE] text-[#C62828] flex items-center justify-center text-lg font-bold">!</span>
                    <div>
            ...
            <div className="bg-white p-8 border border-[#EBE7DE] shadow-lg rounded-xl">
              ...
              <dl className="space-y-6 text-sm">
                <div>
                  ...
                  <dd className="font-serif text-xl text-[#2C2A26]">{study.country}</dd>
                </div>
                <div>
                  <dt className="text-[#A8A29E] text-xs uppercase tracking-wide mb-1">Peak Valuation</dt>
                  <dd className="font-serif text-xl text-[#2C2A26]">{study.peakValuation}</dd>
                </div>
                <div>
                  <dt className="text-[#A8A29E] text-xs uppercase tracking-wide mb-1">Key Year</dt>
                  <dd className="font-serif text-xl text-[#2C2A26]">{study.year}</dd>
                </div>
              </dl>
              ...
              <ul className="space-y-3">
                {study.founders.map((founder, idx) => (
                  <li key={idx} className="flex items-center gap-3 text-[#2C2A26] font-medium">
              ...
              <div className="space-y-0 border-l-2 border-[#D6D1C7] ml-2">
                {study.timeline.map((item, idx) => (
                  <div key={idx} className="relative pl-8 pb-8 last:pb-0">
                    ...
                    <span className="block text-xs font-bold text-[#A8A29E] mb-1 uppercase tracking-wider">{item.year}</span>
                    <span className="block text-sm text-[#2C2A26] font-medium leading-tight">{item.event}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-[#2C2A26] p-8 rounded-xl text-[#F5F2EB] shadow-xl">
              ...
              <ul className="space-y-6">
                {study.lessonsLearned.map((lesson, idx) => (
                  <li key={idx} className="text-lg font-serif italic leading-relaxed opacity-90">
                    "{lesson}"
                  </li>
                ))}
              </ul>
